...
public final class WcCache {
    ...
    private static WcBase buildWxBase(String tenantId){
        WxConfig wxConfig = WxConfigService.getConfigByTenantId(tenantId);
        Assert.notNull(wxConfig, "The wxConfig of " + tenantId);
        
        return new WcBase(wxConfig.getStr("wx_no"),
                          wxConfig.getStr("name"),
                          wxConfig.getStr("token"),
                          wxConfig.getStr("domain"),
                          wxConfig.getStr("origin_id"),
                          wxConfig.getStr("app_id"),
                          wxConfig.getStr("app_secret"),
                          wxConfig.getStr("aes_key"),
                          0);// TODO wx_config 添加字段 明文模式
    }
    ...
    private static AccessToken getRemoteAccessToken(String tenantId) {
        Token rs = Token.dao.getToken(tenantId);
     ...
     * 1：Admin和Front 在AccessToken同时过期竞争状态下会出现Cache中的无效
     * 2：使用微信公众平台接口调试工具获取了该公众号的 access_token
