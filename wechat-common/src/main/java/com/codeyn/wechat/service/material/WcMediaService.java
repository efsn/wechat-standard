...
public class WcMediaService {
    ...
    public static String upload(String tenantId, MediaType type, File file, boolean isForever) {
        ...
        if (isForever) {
            media = getClient(tenantId).uploadMedia(type, file, accessToken, false);
        } else {
            media = getClient(tenantId).uploadMedia(type, file, accessToken, true);
        }
